...
Page({
  ...
  onLoad(options) {
    ...
    const decoratedDecision = this.decorateDecision(decision)
    this.setData({
      decision: decoratedDecision,
      stageMeta: decision ? util.getStageMeta(decision.stage) : {},
      canReview: this.canShowReviewForm(decoratedDecision),
  ...
  decorateDecision(decision) {
    ...
    const maxWaterings = decision.maxWaterings || 1
    const remainingWaterings = Math.max(maxWaterings - wateringHistory.length, 0)
    return Object.assign({}, decision, {
      remainingWaterings,
      hasRemainingFollowUps: decision.status === 'pending' && remainingWaterings > 0
    })
  },
  ...
  onMarkActionStarted() {
    ...
    if (this.saveDecision(updatedDecision)) {
      this.setData({
        decision: this.decorateDecision(updatedDecision),
  ...
  saveReviewResult(extraChoice) {
    ...
    if (isDone) {
      updatedDecision = Object.assign({}, decision, {
        status: reviewType === 'result' ? 'reviewed' : 'pending',
        reviewStage: reviewType === 'result' ? 'result_done' : 'current_done',
        ...
        maxWaterings,
        lastWateredAt: today,
        reviewDate: reviewType === 'result' ? decision.reviewDate : util.addDays(today, 7)
      })
    } else {
  ...
  canShowReviewForm(decision) {
    ...
    if (decision.status === 'pending' && decision.reviewStage !== 'current_done') return true
  ...
  saveDecision(updatedDecision) {
    ...
    const saved = util.safeSetStorage('decisions', decisions)
    if (saved && typeof app.refreshStats === 'function') app.refreshStats()
    return saved
  }
})

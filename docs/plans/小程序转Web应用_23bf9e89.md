...
本次迁移的第一目标不是重做产品，而是把“小程序壳”换成“Web 壳”，并为后续本地保存 + 云端同步两套数据方案打好接口。

## 2. 关键原则

- 先本地可用，再云端同步。V1 只实现 localStorage，V1.5/V2 再接云端。
- 不强制登录。Web 版默认直接进入花园，昵称/云同步均作为后置可选项。
- 数据层不能写死 localStorage。必须通过 Storage Adapter 访问数据，避免后续云同步返工。
...
注意：当前小程序已存在 `cloud-init.js`、`sync-engine.js` 和 `sync-up/sync-down/sync-full` 云函数。Web 端不能直接复用 `wx.cloud.callFunction`，后续云同步需要另选 Web 可用方案，例如 CloudBase Web SDK、自建 API、Serverless Functions 或第三方后端。
...
Web 端建议比小程序更早加入“导出/导入 JSON 备份”，作为云同步前的低成本安全网：
...
先创建 `CloudStorageAdapter.stub.js`，只定义接口，不实现真实请求：

```js
cloudStorage.enable()
cloudStorage.disable()
cloudStorage.pull()
cloudStorage.push(operations)
cloudStorage.getStatus()
```

后续云同步可选方案：

- CloudBase Web SDK：更接近微信云开发，但需要确认 Web 登录和权限方案。
- 自建 API + 数据库：更通用，适合以后扩展账号系统。
- Serverless Functions：适合轻量同步，不需要完整后端工程。
...
| `wx.cloud.callFunction` | V2 Cloud Adapter，不在 V1 实现 |
